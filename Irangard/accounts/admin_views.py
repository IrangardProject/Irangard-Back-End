...
class AdminViewSet(GenericViewSet):
    ...
    def removeSpecialUser(self, request):
        try:
            sp_user = SpecialUser.objects.get(user = request.user)
            sp_user.delete()
            user = User.objects.get(username = request.user.username)
            user.is_special = False 
            user.save()
            return Response(f'special user with username {request.user.username} deleted',status=status.HTTP_200_OK)
        except SpecialUser.DoesNotExist:
            return Response(f'special user with username {request.user.username} doesn not exist',status=status.HTTP_400_BAD_REQUEST)
